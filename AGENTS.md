...
1. `[docs/PRD.md](./docs/PRD.md)` — 完整设计:为什么这么搭、各 phase 拆分、14 个验收用例、17 条风险矩阵
...
- ✅ 8+ 条发布工程经验 + 17 条风险矩阵入档(参见 `docs/CHANGELOG.md` 末尾段 + `docs/PRD.md` §9)
...
详见 `[docs/CHANGELOG.md](./docs/CHANGELOG.md)` 末尾「发布工程经验」段(共 8 条经验)。
3 个高频要点:
...
A: 先看 `docs/CHANGELOG.md` rc.1-rc.6 deprecation 章节 + 8 条经验段,90% 概率是历史遗坑复现。
如果是新坑,按下面流程诊断:
