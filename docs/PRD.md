...
| R11 | pnpm workspace `workspace:*` 协议被原样发布到 npm（dependencies 内）                                                                                                                 | 用户端 `npm i` 触发 EUNSUPPORTEDPROTOCOL 失败                 | release.mjs `withPublishableManifest()` 在 publish 前临时删除 devDependencies + scripts;`workspace:*` 仅留在开发态 devDependencies 内                                                                                                                     |
| R12 | ESM 入口 `isDirectRun` 在 npm bin symlink 启动场景下失效                                                                                                                             | 装好后 cli 所有命令静默 exit 0                                | 删除 isDirectRun 判断,cli.mjs 作为单一 entry 直接调 main();单测/import 用 `AIPT_SKIP_AUTOSTART` env 守卫                                                                                                                                                  |
| R13 | pnpm script 把 `~/.npmrc` 注入 `npm_config_*` env,覆盖 `--userconfig` 文件                                                                                                           | release.mjs 子进程 npm 命令实际 registry / token 不一致 → 401 | publish 链路所有 npm 命令显式同时带 `--userconfig=<abs> --registry=<official>`,CLI flag 优先级最高碾压 env                                                                                                                                                |
| R14 | npm 2FA / GAT 强制策略导致 `npm publish` 403                                                                                                                                         | 首次真发卡住                                                  | 推荐组合:账号开 2FA 保护登录 + 生成 Granular Access Token (Bypass 2FA enabled,scope 级 read-write) 写到项目 `.npmrc.publish`,release.mjs 自动注入                                                                                                         |
| R15 | macOS GUI 应用启子进程时 PATH 不含 nvm / volta / fnm node                                                                                                                            | IDE 启 MCP/Hook 子进程 ENOENT 静默失败                        | 所有写到 IDE 配置(mcp.json / hooks.json / settings.json)的命令必须用 `process.execPath` 绝对路径,不依赖 PATH 解析                                                                                                                                         |
| R16 | stdio MCP server 启动后 `server.connect()` 不阻塞,process.exit 关闭 stdio                                                                                                            | Cursor 报 "MCP error -32000: Connection closed"               | mcp 子命令显式 `return new Promise<number>(() => {})` hang event loop;所有长期运行的 stdio / HTTP server 子命令同款模式                                                                                                                                   |
| R17 | 大型项目迁移时源仓库的死代码 fallback 陷阱(实测命中 4 处 `~/Downloads/...`)                                                                                                          | 测试通过但实际安装/运行时炸出来                               | (1) `grep -rn '<老路径关键字>' packages/` 一次性 sweep;(2) 写"全新机器端到端 e2e"用 mkdtempSync 模拟纯净 HOME 跑完整流程;(3) rc 阶段透明 deprecate 老版本                                                                                                 |
| R18 | Cursor `stop` hook 在用户手动中断(ESC/Cancel)时**也会触发**,payload `status='aborted'/'error'`;若直接 inject_followup,Cursor 会自动 submit 为新 prompt → LLM 被强制重答,违背用户意图 | 用户感知"我已经中断了,但 stop-check 还把我打回来",体验极差    | `runStopCheck()` 内 `detectDialect()` 之后立即按 status 过滤,`aborted`/`error` 返回新 outcome `skipped_aborted`,绝不输出 followup。Claude Code 文档明确 Stop hook "do not fire on user interrupts" 不受影响,Cursor 必须主动处理。详 CHANGELOG 第 9 条经验 |
